...
import JumpSequncePreview from "./JumpSequencePreview";
import { decodeHtmlEntities } from "utils/common.util";
import { QuestionSetup } from "apiEndpoints/QuestionSetup";

function PreviewSurveys({
  surveyID,
  companyID,
  setPreviewQuestionSetup,
  isDataByProps = false,
  setShowDataAnalytics,
}) {
  ...
  const fetchOutcome = async () => {
    try {
      ...
      const response = await commonService({
        ...
        queryParams: {
          companyID: isDataByProps ? companyID : surveyData.companyID,
          surveyID: isDataByProps ? surveyID : surveyData.surveyID,
        },
        headers: {
                    ...
                    <div className="d-flex justify-content-between actionBtn mt-xl-5 mt-4 mb-xl-5 mb-4">
                      ...
                      <div className="d-flex gap-2">
                        ...
                        {currentPage === totalPages ? (
                          <Button
                            className="btn btn-primary questionPreviewBtn"
                            onClick={() => handleSubmit(true)} // Pass true for final submit
                          >
                            {nextIsSubmitting ? (
                              <>
                                <Spinner
