...
import {
  SingleRatingQuestionValidationSchemaEditIG,
  SingleRatingQuestionValidationSchemaIG,
} from "../../../validation";
...
export default function RatingQuestionIG({
  setActiveForm,
  outcome,
  surveyType,
  surveyID,
    ...
    async (values, { resetForm }) => {
      try {
        ...
        const payload = {
          ...
          allResponse: values.responses.map((item) => ({
            response: item.response,
            isOEQ: item.hasOeq ? 1 : 0,
            oeqQuestion: item.hasOeq ? item.openEndedQuestion : "",
          })),
        ...
        // Add questionID for update operation
        if (initialQuestionData?.questionID) {
        ...
        console.error("Error updating/adding IG question:", error);
        setIsSubmitting(false);
      }
    },
    [
  ...
  // Get initial values for IG form
  const getInitialValues = () => {
    if (!initialQuestionData || initialQuestionData.isScore === 1) {
      return {
        question: "",
        intentionName: "",
        intentionShortName: "",
        displaySkipForNow: false,
        responseViewOption: "vertical",
    ...
    return {
      ...
      intentionName: initialQuestionData.intentionName || "",
      intentionShortName: initialQuestionData.intentionShortName || "",
      ...
      addToResource: initialQuestionData.isResponseAddedToResource || false,
      responseBlockName:
      ...
      isResponseAddedToResource:
        initialQuestionData.isResponseAddedToResource || false,
    };
  };
  ...
  const formikConfig = useMemo(
    ...
    [initialQuestionData]
  ); // Add initialQuestionData as dependency
  ...
  const handleCheckboxChangeOeq = useCallback(
    (index) => {
  ...
  const handleAddResponseBlockToResourceTitle = () => {
    if (formik.values.responses.length > 0) {
      console.log(formik.values.responses[0]?.response);
      // console.log(formik.values.responses[0]?.response);
            ...
            {formik.values.responses.map((response, index) => (
              ...
              >
                <div className="scalarappend_list d-flex justify-content-between gap-2 align-items-start">
                  ...
                  <Form.Group className="form-group scalar w-75">
                    <InputField
                      type="text"
