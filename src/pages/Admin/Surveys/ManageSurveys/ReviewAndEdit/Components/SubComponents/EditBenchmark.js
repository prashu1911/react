...
const EditBenchmarkComp = ({
  ...
  useEffect(() => {
    if (type === "edit") getQuestionsForSurvey();
  }, [benchmarkID, type]);
  ...
  const handleEditBenchmark = async () => {
    setIsSubmitting(true);
    try {
      if (benchmarkName && benchmarkName.trim() !== "") {
        const response = await commonService({
          apiEndPoint: SURVEYS_MANAGEMENT.updateBenchmark,
          bodyData: {
...
export default EditBenchmarkComp;
