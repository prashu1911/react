...
const ContactFilterModal = ({
  ...
  function convertApiDataToSampleData(apiData) {
    return apiData.map((item, index) => ({
      id: index + 1,
      faqID: index + 1,
      filterhead: item.contactTitle,
      filterpara: item.contactDescription,
      checkboxData: false,
    }));
  }

  const handleContactDefaultSetting = async () => {
    const response = await commonService({
      apiEndPoint: SURVEYS_MANAGEMENT.contactDefaultSetting,
      queryParams: {
        companyID,
      },
      headers: {
    ...
    if (response?.status) {
      let testData = Array.isArray(response?.data?.data?.contactContent) ? response?.data?.data?.contactContent : [] ;
  ...
  function updateCheckedItems(faqListData, selectedFaq, checkedItemsData) {
    ...
    for (const item of faqListData) {
      ...
      for (const selectedItem of selectedFaq) {
        if (item.faqID === selectedItem.contactID) {
  ...
  function convertApiDataToSampleData2(apiData) {
    ...
    for (let oneRow of apiData) {
      if (oneRow.contactID) {
        let data = {
          id: index,
          faqID: oneRow.contactID,
          filterhead: oneRow.help_title,
          filterpara: oneRow.help_description,
          checkboxData: true,
        };
