...
export default function ErrorBarChart({ updateSection, SectionId, SectionData }) {
    ...
    const tooltipMessage="Error Bar Charts are restricted to 3 comparisons"
    ...
    const [formData, setFormData] = useState({
        ...
        selectedPalletteID: "",
        objectControlList: "",
        selectedErrorBarDatasets: [],
        comparisonType:'datapoint'
    });
    ...
    const limitSelectedToTwo = (dataPointControlList) => {
        ...
        // Create a shallow clone so we don't mutate the original object
        const updated = {
            ...dataPointControlList,
            overall: { ...dataPointControlList.overall },
        };

        if (updated.overall?.selected && selectedCounter > 3) {
    ...
    const handleComparisonChange = async (type) => {
        ChangesTrue()
        if (type === 'object') {
            ...
            const selectedErrorBarDatasets = [];

            updatedData.forEach((outcome) => {
                if (outcome.selected) {
                    selectedErrorBarDatasets.push({
    ...
    const handleDataPointModal = (value) => {
        ...
        if (formData?.comparisonType === "datapoint") {
            setTimeout(() => {
                handleComparisonChange("datapoint");
            }, 100);
        }
    };
    ...
    const handleDragEnd = (result) => {
        ...
        const items = Array.from(formData.selectedErrorBarDatasets);
        const [reorderedItem] = items.splice(result.source.index, 1);
        items.splice(result.destination.index, 0, reorderedItem);

        setFormData((prev) => ({
            ...prev,
            selectedErrorBarDatasets: items,
        }));
    };

    const handleControlChange = (selectedId) => {
        ChangesTrue()
        const updated = formData?.selectedErrorBarDatasets?.map((item) => ({
            ...item,
            control: item.id === selectedId
        }));
        setFormData((prev) => ({
            ...prev,
            selectedErrorBarDatasets: updated
        }));
    };
    ...
    const handleSubmit = async () => {
        ...
        const hasControlTrue = formData.selectedErrorBarDatasets?.some(item => item.control === true);
        ...
        if (!hasControlTrue) {
            showErrorToast("At least one selected Dataset must have control set to true.");
            return;
        }
        if (!formData?.title) {
    ...
    return (
        <div className="mt-xl-4 mt-3">
            {ShowDataPointModal &&
                <DataPointModal ChangesTrue={ChangesTrue} limit={formData?.comparisonType === "datapoint" ? 3 : 0} ShowDataPointModal={ShowDataPointModal} setShowDataPointModal={handleDataPointModal} formData={formData} setFormData={setFormData} dataPointControlList={formData?.dataPointControlList} />
            }
            {ShowObjectPointModal &&
                <ObjectPointModal ChangesTrue={ChangesTrue} limit={formData?.comparisonType === "object" ? 3 : 0} ShowObjectPointModal={ShowObjectPointModal} setShowObjectPointModal={handleObjectPointModal} formData={formData} setFormData={setFormData} objectControlList={formData?.objectControlList} />
            }
            ...
            {(formData?.comparisonType === "datapoint" || formData?.comparisonType==null) &&
                <>
                    <div style={{ backgroundColor: '#E5E5E6', padding: '0.5rem', paddingBottom: '1px', borderRadius: '0.5rem', marginBottom: '0.5rem' }}>
                        ...
                        <Form.Group className="form-group" >
                            ...
                            {formData?.selectedErrorBarDatasets.length > 0 && formData?.comparisonType === "datapoint" &&
                                        ...
                                        <Droppable droppableId="tornado-list">
                                            {(provided) => (
                                                <div {...provided.droppableProps} ref={provided.innerRef}>
                                                    {formData?.selectedErrorBarDatasets?.map((item, index) => (
                    ...
                    <Form.Group className="form-group" >
                        ...
                        {formData?.selectedErrorBarDatasets.length > 0 && formData?.comparisonType === "object" &&
                            <div className="bg-light p-3 rounded" style={{ maxWidth: "330px" }}>
                                <div className="d-flex justify-content-between align-items-center mb-2">
                                    <strong>Selected Data Points</strong>
                                    <div style={{ display: 'flex' }}>
